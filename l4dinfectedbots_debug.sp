...
#define DEBUGSERVER 1  // 改为1查看服务器debug
#define DEBUGCLIENTS 1  // 改为1查看对话框debug
#define DEBUGTANK 0
#define DEBUGHUD 0
...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_on14", R14Infectedon, "开启16特模式", 0);
	RegConsoleCmd("sm_on142", R14Infectedon2, "开启每人两特模式", 0);
	RegConsoleCmd("sm_on141", R14Infectedon3, "开启按配置增加特感模式", 0);
	...
	h_BoomerLimit = CreateConVar("l4d_infectedbots_boomer_limit", "3", "设置插件产生boomers的上限", FCVAR_SPONLY);
	h_SmokerLimit = CreateConVar("l4d_infectedbots_smoker_limit", "3", "设置插件产生smokers的上限", FCVAR_SPONLY);
	...
	if (b_IsL4D2)
	{
		h_SpitterLimit = CreateConVar("l4d_infectedbots_spitter_limit", "3", "设置插件产生spitters的上限", FCVAR_SPONLY);
		h_JockeyLimit = CreateConVar("l4d_infectedbots_jockey_limit", "3", "设置插件产生jockeys的上限", FCVAR_SPONLY);
		h_ChargerLimit = CreateConVar("l4d_infectedbots_charger_limit", "3", "设置插件产生chargers的上限", FCVAR_SPONLY);
		h_HunterLimit = CreateConVar("l4d_infectedbots_hunter_limit", "3", "设置插件产生hunters的上限", FCVAR_SPONLY);
	}
	else
	{
		h_HunterLimit = CreateConVar("l4d_infectedbots_hunter_limit", "3", "设置插件产生hunters的上限", FCVAR_SPONLY);
	}
	h_MaxPlayerZombies = CreateConVar("l4d_infectedbots_max_specials", "16", "定义在所有游戏模式下地图上可能有多少特殊感染者（这也会影响感染者玩家的上限）", FCVAR_SPONLY); 
	h_InfectedSpawnTimeMax = CreateConVar("l4d_infectedbots_spawn_time_max", "30", "设置插件产生的特殊感染的最大产生时间（以秒为单位）", FCVAR_SPONLY);
	h_InfectedSpawnTimeMin = CreateConVar("l4d_infectedbots_spawn_time_min", "25", "设置插件产生的特殊感染的最短产生时间（以秒为单位）", FCVAR_SPONLY);
	...
	h_idletime_b4slay = CreateConVar("l4d_infectedbots_lifespan", "40", "踢出特殊感染者的秒数（感染者生存周期）", FCVAR_SPONLY);
	h_InitialSpawn = CreateConVar("l4d_infectedbots_initial_spawn_timer", "1", "第一次在地图上生成感染者电脑时的生成计时器秒数", FCVAR_SPONLY);
...
{
	PrintToChatAll("RAScheck:%d, min:%d, max:%d, si:%d",RAScheck,InfectedSpawnTimeMin,InfectedSpawnTimeMax,MaxPlayerZombies);
	PrintToChatAll("RIFADDNUMS:%d, baseNum:%d, IFADDEnabled:%d, R14Enabled:%d",RIFADDNUMS,baseNum,IFADDEnabled,R14Enabled);
	PrintToServer("RAScheck:%d, min:%d, max:%d, si:%d",RAScheck,InfectedSpawnTimeMin,InfectedSpawnTimeMax,MaxPlayerZombies);
	PrintToServer("RIFADDNUMS:%d, baseNum:%d, IFADDEnabled:%d, R14Enabled:%d",RIFADDNUMS,baseNum,IFADDEnabled,R14Enabled);
}
...
Rs14Infectedon()
{
	...
	R14Enabled = true;
	EnabledCheck();
	PrintToChatAll("\x04[!警告!]\x05 开启了\x04 %d \x05特模式,请注意!关闭请输入!off14", MaxPlayerZombies);
	return 0;
}
...
public Action:R14Infectedoff(client, args)
{
	...
	R14Enabled = false;
	EnabledCheck();
	PrintToChatAll("\x04[!警告!]\x05 关闭了\x04 %d \x05特模式,请注意!开启请输入!on14 ", MaxPlayerZombies);
	return Action:0;
}
...
public void SetItemNum(int itemNum)
{
	...
	PrintToChatAll("\x04[!警告!]\x05 特感重生时间(秒)\x04 Min:%d - Max:%d\x05,请注意!", SpawnTimeMin, SpawnTimeMax);
}
...
public Action:ADDIFNUMCHECKSD(Handle:timer, any:Rflag)
{
	...
	if (Rflag == 3)
	{
		PrintToChatAll("\x04[!警告!]\x05开启了\x04%d\x05特模式按人数增加,最少\x04%d\x05特,每增加一名幸存者增加\x04%d\x05特,关闭请输入!off14", MaxPlayerZombies, baseNum, RIFADDNUMS);
	}
	else
	{
...
public Action:Announce_Delay(Handle:timer, any:Rflag)
{
	switch (Rflag)
	{
		case 0:
		{
			PrintToChatAll("\x04[!提示!]\x05 特感数量增加现在是\x03 %d 特.", MaxPlayerZombies);
		}
		case 1:
		{
			PrintToChatAll("\x04[!提示!]\x05 -幸存者减少了,特感数量现在是\x03 %d 特.", MaxPlayerZombies);
		}
		case 2:
		{
			PrintToChatAll("\x04[!提示!]\x05 +幸存者增加了,特感数量现在是\x03 %d 特.", MaxPlayerZombies);
		}
		default:
		{
		}
