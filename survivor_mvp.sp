...
public OnPluginStart()
{
    ...
    // Cvars
    hPluginEnabled =    CreateConVar("sm_survivor_mvp_enabled", "1", "Enable display of MVP at end of round", 0, true, 0.0, true, 1.0);
    hCountTankDamage =  CreateConVar("sm_survivor_mvp_counttank", "0", "Damage on tank counts towards MVP-selection if enabled.", 0, true, 0.0, true, 1.0);
    hCountWitchDamage = CreateConVar("sm_survivor_mvp_countwitch", "0", "Damage on witch counts towards MVP-selection if enabled.", 0, true, 0.0, true, 1.0);
    hTrackFF =          CreateConVar("sm_survivor_mvp_showff", "1", "Track Friendly-fire stat.", 0, true, 0.0, true, 1.0);
    hBrevityFlags =     CreateConVar("sm_survivor_mvp_brevity", "0", "Flags for setting brevity of MVP report (hide 1:SI, 2:CI, 4:FF, 8:rank, 32:perc, 64:abs).", 0, true, 0.0);
...
public Action:ShowMVPStats_Cmd(client, args)
{
    ...
    if (client && IsClientConnected(client))
    {
        ...
        if (!(iBrevityFlags & BREV_CI))
        {
            ...
            {
                if (iBrevityFlags & BREV_PERCENT) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 个人 - 普通感染者: (\x05%d \x01common)\n", iGotCommon[client]);
                } else if (iBrevityFlags & BREV_ABSOLUTE) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 个人 - 普通感染者: (\x04%.0f%%\x01)\n", (float(iGotCommon[client]) / float(iTotalCommon)) * 100);
                } else {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 个人 - 普通感染者: (\x05%d \x01common [\x04%.0f%%\x01])\n", iGotCommon[client], (float(iGotCommon[client]) / float(iTotalCommon)) * 100);
                }
                StrCat(printBuffer, sizeof(printBuffer), tmpBuffer);
            }
        }
        ...
        if (!(iBrevityFlags & BREV_FF) && bTrackFF) {
            PrintToChat(client, "\x01个人 - 队友伤害: (\x05%d \x01队友伤害 [\x04%.0f%%\x01])\n", iDidFF[client], (float(iDidFF[client]) / float(iTotalFF)) * 100);
        }
    ...
    if (iTotalDamageAll > 0 && !(iBrevityFlags & BREV_RANK) && !(iBrevityFlags & BREV_SI))
    {
        ...
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_SI_losers[i]) && !IsFakeClient(mvp_SI_losers[i])) {
                if (iBrevityFlags & BREV_PERCENT) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 你的排名 - 特殊感染者: #\x03%d \x01(\x05%d \x01伤害,\x05 %d \x01击杀)", (i + 2), iDidDamageAll[mvp_SI_losers[i]], iGotKills[mvp_SI_losers[i]]);
                } else if (iBrevityFlags & BREV_ABSOLUTE) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 你的排名 - 特殊感染者: #\x03%d \x01(伤害 \x04%.0f%%\x01, 击杀 \x04%.0f%%\x01)", (i + 2), (float(iDidDamageAll[mvp_SI_losers[i]]) / float(iTotalDamageAll)) * 100, (float(iGotKills[mvp_SI_losers[i]]) / float(iTotalKills)) * 100);
                } else {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 你的排名 - 特殊感染者: #\x03%d \x01(\x05%d \x01伤害 [\x04%.0f%%\x01],\x05 %d \x01击杀 [\x04%.0f%%\x01])", (i + 2), iDidDamageAll[mvp_SI_losers[i]], (float(iDidDamageAll[mvp_SI_losers[i]]) / float(iTotalDamageAll)) * 100, iGotKills[mvp_SI_losers[i]], (float(iGotKills[mvp_SI_losers[i]]) / float(iTotalKills)) * 100);
                }
    ...
    if (iTotalCommon > 0 && !(iBrevityFlags & BREV_RANK) && !(iBrevityFlags & BREV_CI))
    {
        ...
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_CI_losers[i]) && !IsFakeClient(mvp_CI_losers[i])) {
                if (iBrevityFlags & BREV_PERCENT) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 你的排名 - 普通感染者: #\x03%d \x01(\x05%d \x01击杀)", (i + 2), iGotCommon[mvp_CI_losers[i]]);
                } else if (iBrevityFlags & BREV_ABSOLUTE) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 你的排名 - 普通感染者: #\x03%d \x01(击杀 \x04%.0f%%\x01)", (i + 2), (float(iGotCommon[mvp_CI_losers[i]]) / float(iTotalCommon)) * 100);
                } else {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 你的排名 - 普通感染者: #\x03%d \x01(\x05%d \x01击杀 [\x04%.0f%%\x01])", (i + 2), iGotCommon[mvp_CI_losers[i]], (float(iGotCommon[mvp_CI_losers[i]]) / float(iTotalCommon)) * 100);
                }
    ...
    if (iTotalFF > 0 && bTrackFF && !(iBrevityFlags & BREV_RANK) && !(iBrevityFlags & BREV_FF))
    {
        ...
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_FF_losers[i]) && !IsFakeClient(mvp_FF_losers[i])) {
                if (iBrevityFlags & BREV_PERCENT) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[LVP] 你的排名 - 队友伤害: #\x03%d \x01(\x05%d \x01伤害)", (i + 2), iDidFF[mvp_FF_losers[i]]);
                } else if (iBrevityFlags & BREV_ABSOLUTE) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[LVP] 你的排名 - 队友伤害: #\x03%d \x01(伤害 \x04%.0f%%\x01)", (i + 2), (float(iDidFF[mvp_FF_losers[i]]) / float(iTotalFF)) * 100);
                } else {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[LVP] 你的排名 - 队友伤害: #\x03%d \x01(\x05%d \x01伤害 [\x04%.0f%%\x01])", (i + 2), iDidFF[mvp_FF_losers[i]], (float(iDidFF[mvp_FF_losers[i]]) / float(iTotalFF)) * 100);
                }
...
String: GetMVPString()
{
    ...
    {
        Format(tmpBuffer, sizeof(tmpBuffer), "MVP: 还没有足够的数据哦~\n");
        StrCat(printBuffer, sizeof(printBuffer), tmpBuffer);
    }
    else
    {
        if (!(iBrevityFlags & BREV_SI))
        {
            ...
            {
                if (iBrevityFlags & BREV_PERCENT) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 特殊感染者:\x03 %s \x01(\x05%d \x01伤害,\x05 %d \x01击杀)\n", mvp_SI_name, iDidDamageAll[mvp_SI], iGotKills[mvp_SI]);
                } else if (iBrevityFlags & BREV_ABSOLUTE) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 特殊感染者:\x03 %s \x01(伤害 \x04%2.0f%%\x01, 击杀 \x04%.0f%%\x01)\n", mvp_SI_name, (float(iDidDamageAll[mvp_SI]) / float(iTotalDamageAll)) * 100, (float(iGotKills[mvp_SI]) / float(iTotalKills)) * 100);
                } else {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 特殊感染者:\x03 %s \x01(\x05%d \x01伤害[\x04%.0f%%\x01],\x05 %d \x01击杀 [\x04%.0f%%\x01])\n", mvp_SI_name, iDidDamageAll[mvp_SI], (float(iDidDamageAll[mvp_SI]) / float(iTotalDamageAll)) * 100, iGotKills[mvp_SI], (float(iGotKills[mvp_SI]) / float(iTotalKills)) * 100);
                }
                StrCat(printBuffer, sizeof(printBuffer), tmpBuffer);
            }
            else
            {
                StrCat(printBuffer, sizeof(printBuffer), "[MVP] 特殊感染者: \x03(无人)\x01\n");
            }
        }
        ...
        if (!(iBrevityFlags & BREV_CI))
        {
            ...
            {
                if (iBrevityFlags & BREV_PERCENT) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 普通感染者:\x03 %s \x01(\x05%d \x01普通感染者)\n", mvp_Common_name, iGotCommon[mvp_Common]);
                } else if (iBrevityFlags & BREV_ABSOLUTE) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 普通感染者:\x03 %s \x01(\x04%.0f%%\x01)\n", mvp_Common_name, (float(iGotCommon[mvp_Common]) / float(iTotalCommon)) * 100);
                } else {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[MVP] 普通感染者:\x03 %s \x01(\x05%d \x01普通感染者 [\x04%.0f%%\x01])\n", mvp_Common_name, iGotCommon[mvp_Common], (float(iGotCommon[mvp_Common]) / float(iTotalCommon)) * 100);
                }
                StrCat(printBuffer, sizeof(printBuffer), tmpBuffer);
            }
        }
    ...
    if (!(iBrevityFlags & BREV_FF) && bTrackFF)
    {
        if (mvp_FF == 0)
        {
            Format(tmpBuffer, sizeof(tmpBuffer), "LVP - 队友伤害: 竟然完全没有!\n");
            StrCat(printBuffer, sizeof(printBuffer), tmpBuffer);
        }
        else
        {
            if (iBrevityFlags & BREV_PERCENT) {
                Format(tmpBuffer, sizeof(tmpBuffer), "[LVP] 队友伤害:\x03 %s \x01(\x05%d \x01伤害)\n", mvp_FF_name, iDidFF[mvp_FF]);
            } else if (iBrevityFlags & BREV_ABSOLUTE) {
                Format(tmpBuffer, sizeof(tmpBuffer), "[LVP] 队友伤害:\x03 %s \x01(\x04%.0f%%\x01)\n", mvp_FF_name, (float(iDidFF[mvp_FF]) / float(iTotalFF)) * 100);
            } else {
                Format(tmpBuffer, sizeof(tmpBuffer), "[LVP] 队友伤害:\x03 %s \x01(\x05%d \x01伤害 [\x04%.0f%%\x01])\n", mvp_FF_name, iDidFF[mvp_FF], (float(iDidFF[mvp_FF]) / float(iTotalFF)) * 100);
            }
            StrCat(printBuffer, sizeof(printBuffer), tmpBuffer);
        }
    }
