...
public void OnPluginStart()
{
	...
	g_hMenu.SetTitle("%s", "Hat_Menu_Title");
...
public Action CmdHat(int client, int args)
{
	...
	{
		CPrintToChat(client, "%s%s", CHAT_TAG, "No Access");
	...
	if( args == 1 )
	{
		...
		if( strlen(sTemp) < 3 )
		{
			...
			{
				CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_No_Index", index, g_iCount);
			}
			else
			{
				RemoveHat(client);
		...
		else
		{
			...
			CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_Not_Found", sTemp);
		}
	}
	else
	{
	...
	else if( action == MenuAction_Select )
	{
		...
		if( target )
		{
			...
			if( IsValidClient(target) )
			{
				...
				CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_Changed", name);
				RemoveHat(target);
			...
			{
				CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_Invalid");
			}
...
void ShowMenu(int client)
{
	...
	else
	{
		...
		hTemp.SetTitle("%s", "Hat_Menu_Title", client);

		for( int i = 0; i < g_iCount; i++ )
		{
			Format(sTemp, sizeof(sTemp), "%s", g_sNames[i], client);
...
public Action CmdHatOff(int client, int args)
{
	...
	char sTemp[32];
	Format(sTemp, sizeof(sTemp), "%s", g_bHatOff[client] ? "Hat_Off" : "Hat_On", client);
	CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_Ability", sTemp);
...
public Action CmdHatShow(int client, int args)
{
	...
	{
		CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_Missing");
		return Plugin_Handled;
	}
	...
	char sTemp[32];
	Format(sTemp, sizeof(sTemp), "%s", g_bHatView[client] ? "Hat_On" : "Hat_Off", client);
	CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_View", sTemp);
	return Plugin_Handled;
}
		...
		if( g_bMenuType[client] )
		{
			...
			if( g_bBlocked[target] == false )
			{
				if( IsValidClient(target) )
				{
					...
					char name[MAX_NAME_LENGTH];
					GetClientName(target, name, sizeof(name));
					CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_Unblocked", name);
				}
			}
			else
			{
				...
				CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_Blocked", name);
				RemoveHat(target);
			}
		}
		else
...
public Action CmdHatDel(int client, int args)
{
	...
	else
	{
		...
		if( g_bTranslation == false )
		{
			CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_Wearing", g_sNames[index]);
		}
		else
		{
			char sMsg[128];
			...
			CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_Wearing", sMsg);
		}
	}
	return Plugin_Handled;
}
...
bool CreateHat(int client, int index = -1)
{
	...
	if( entity != -1 )
	{
		...
		else
		{
			char sMsg[128];
			Format(sMsg, sizeof(sMsg), "Hat %d", index + 1);
			Format(sMsg, sizeof(sMsg), "%s", sMsg, client);
			CPrintToChat(client, "%s%s", CHAT_TAG, "Hat_Wearing", sMsg);
		}
