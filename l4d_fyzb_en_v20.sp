...
{
	CreateConVar("l4d_fyzb_version", PLUGIN_VERSION, " ", FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	l4d_fyzb_enabled = CreateConVar("l4d_fyzb_enabled", "1", " 1 : enable , 0: disable ", 0);
	l4d_fyzb_init = CreateConVar("l4d_fyzb_init", "0", " 1 : initial enable , 0: initial disable ", 0);
	l4d_fyzb_speed = CreateConVar("l4d_fyzb_speed", "650.0", "jump speed", 0);
	l4d_fyzb_distancediff = CreateConVar("l4d_fyzb_distancediff", "60.0", "jump difficult >0", 0);
	l4d_fyzb_anglediff1 = CreateConVar("l4d_fyzb_anglediff1", "9.0", "wall jump difficult >=0", 0);
  	l4d_fyzb_damage = CreateConVar("l4d_fyzb_damage", "5", "jump damage for survivor", 0);
 	l4d_fyzb_damage2 = CreateConVar("l4d_fyzb_damage2", "50", "jump damage for infected", 0);
 	l4d_fyzb_movement = CreateConVar("l4d_fyzb_movement", "0.90", "movement slow [0.5-1.0]", 0);
 	l4d_fyzb_safegravity = CreateConVar("l4d_fyzb_safegravity", "0.6", "fall down gravity for safe [0.3-1.0]", 0);
 	l4d_fyzb_safefallspeed = CreateConVar("l4d_fyzb_safefallspeed", "250", "fall down speed great this will use safe gravity[100-1000]", 0);
 	l4d_fyzb_callboss = CreateConVar("l4d_fyzb_callboss", "5", "call infected probility %", 0);
 	l4d_fyzb_showtime = CreateConVar("l4d_fyzb_showtime", "80", "message time", 0);
  	l4d_fyzb_infecteduse = CreateConVar("l4d_fyzb_infecteduse", "0", "0 , disable for infected 1, enable for infected, 3,enable for infected but tank", 0);
  	l4d_fyzb_god = CreateConVar("l4d_fyzb_god", "1", "0  enable fall damage, 1: disable fall damage", 0);
	...
	safegravity=GetConVarFloat(l4d_fyzb_safegravity);
 	...
 	godmode=GetConVarInt(l4d_fyzb_god) ;
  	...
  	all_iVelocity = FindSendPropInfo("CBasePlayer", "m_vecVelocity[0]");
...
public Action:Command_FYZB(client, args)
{
	...
	{
		PrintToChat(client, "\x03飞檐走壁已启用, 按下潜行+跳使用 (shift+space)");
	}
	else
	{
		PrintToChat(client, "\x03飞檐走壁已关闭");
		SetEntityGravity(client,1.0);
		SetEntPropFloat(client, Prop_Data, "m_flLaggedMovementValue",  1.0);
	}
...
public Action:Command_FYZB2(client, args)
{
	...
	if(	JumpEnabled[client])
	{
		PrintToChat(client, "\x03飞檐走壁已启用, 按下下蹲+跳使用 (ctrl+space)");
	}
	else
	{
		PrintToChat(client, "\x03飞檐走壁已关闭");
