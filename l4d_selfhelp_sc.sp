...
public OnPluginStart()
{
	...
	l4d_selfhelp_incap = CreateConVar("l4d_selfhelp_incap", "3", "倒地自救 , 0:禁用, 1:药丸-兴奋剂, 2:药包, 3:两者皆可", 0);
	l4d_selfhelp_grab = CreateConVar("l4d_selfhelp_grab", "3", "被舌头勒住自救 , 0:禁用, 1:药丸-兴奋剂, 2:药包, 3:两者皆可", 0);
	l4d_selfhelp_pounce = CreateConVar("l4d_selfhelp_pounce", "3", "被hunter扑倒自救 , 0:禁用, 1:药丸-兴奋剂, 2:药包, 3:两者皆可", 0);
	l4d_selfhelp_ride = CreateConVar("l4d_selfhelp_ride", "3", "被猴子骑自救, 0:禁用, 1:药丸-兴奋剂, 2:药包, 3:两者皆可", 0);
	l4d_selfhelp_pummel = CreateConVar("l4d_selfhelp_pummel", "3", "被牛抓住自救, 0:禁用, 1:药丸-兴奋剂, 2:药包, 3:两者皆可", 0);
	l4d_selfhelp_edgegrab = CreateConVar("l4d_selfhelp_edgegrab", "3", "悬崖自救, 0:禁用, 1:药丸-兴奋剂, 2:药包, 3:两者皆可", 0);
	l4d_selfhelp_eachother = CreateConVar("l4d_selfhelp_eachother", "1", "倒地互相自救, 0: 禁用, 1 :开启", 0);
	l4d_selfhelp_pickup = CreateConVar("l4d_selfhelp_pickup", "1", "倒地捡东西, 0: 禁用, 1 :开启", 0);
	l4d_selfhelp_kill = CreateConVar("l4d_selfhelp_kill", "1", "自救后杀死攻击者", 0);

	l4d_selfhelp_hintdelay = CreateConVar("l4d_selfhelp_hintdelay", "1.0", "提示延迟", 0);
	l4d_selfhelp_delay = CreateConVar("l4d_selfhelp_delay", "1.0", "延迟多久后才可自救", 0);
	l4d_selfhelp_durtaion = CreateConVar("l4d_selfhelp_durtaion", "5.0", "自救时间", 0);

	l4d_selfhelp_versus = CreateConVar("l4d_selfhelp_versus", "1", "对抗模式开启, 0: 禁用, 1 :开启", 0);	
...
public Action:AdvertisePills(Handle:timer, any:client)
{
	...
	{
		CPrintToChat(client, "{default}[{red}自救{default}] 按住[\x03Ctrl{default}] 自救");
	}
...
public Action:PlayerTimer(Handle:timer, any:client)
{
	...
	if(haveone>0)
	{
		if((buttons & IN_DUCK) ||  (buttons & IN_USE)) 
		{
			if(CanSelfHelp(client))
			{
				...
				else
				{
					...
					ShowBar(client,"自救 ", time-HelpStartTime[client], GetConVarFloat(l4d_selfhelp_durtaion));
				}
		...
		else
		{
			if(HelpState[client]==STATE_SELFHELP)
			{
				...
				{
					ShowBar(client, "自救 ", 0.0, GetConVarFloat(l4d_selfhelp_durtaion));
				}
				HelpState[client]=STATE_NONE;
			}
	...
	else if(GetConVarInt(l4d_selfhelp_eachother)>0)
	{
		...
		if ((buttons & IN_DUCK) ||  (buttons & IN_USE)) 
		{
			...
			{
				decl String:msg[30];
				Format(msg, sizeof(msg), "正在帮助 %N", other);
				if(HelpOhterState[client]==STATE_NONE)
				{
					...
					PrintHintText(other, "%N 在帮助你", client);
					HelpStartTime[client]=time;
				}
			...
			else
			{
				...
				{
					if(L4D2Version) KillProgressBar(client);
					else ShowBar(client, "帮助队友 ", 0.0, GetConVarFloat(l4d_selfhelp_durtaion));
				}
				HelpOhterState[client]=STATE_NONE;
...
SelfHelp(client)
{
	...
	if(slot!=-1)
	{
		...
		if(slot ==4)
		{
			...
			if(adrenaline)	CPrintToChatAll("{default}[{blue}自救{default}] {blue}%N{default} 用兴奋剂自救!", client); 
			if(pills)	CPrintToChatAll("{default}[{blue}自救{default}] {blue}%N{default} 用止痛药自救!", client);
		...
		else if(slot==3)
		{
			...
			HelpState[client]=STATE_OK;
			CPrintToChatAll("{default}[{blue}自救{default}] {blue}%N{default} 用医疗包自救!", client);
 ...
 HelpOther(client, helper)
{
	...
	CPrintToChatAll("{default}[{blue}自救{default}] {blue}%N{default} 倒地后救起了{blue} %N", helper, client); 
	//first(client);
 }
ReviveClientWithKid(client)
{
