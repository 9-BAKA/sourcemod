...
new String:EN_name[10][500][64];
new String:CHI_name[10][500][64];
...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_mapvote", Command_VotemapsMenu);
	...
	while (i < 500)
	{
		...
		{
			KvGetString(hFile, "中文名", CHI_name[0][i], 64, "");
			KvGetString(hFile, "建图代码", EN_name[0][i], 64, "");
			KvGetString(hFile, "类别", catalog, 3, "");
...
public Handler_VoteCallback(Handle:menu, MenuAction:action, param1, param2)
{
	...
	{
		if ((strcmp(item, VOTE_YES) == 0 && votes * 2 <= playernum && param1 == 0) || (strcmp(item, VOTE_NO) == 0 && param1 == 1))
		{
