...
public OnPluginStart()
{
	...
	g_hCvar_ReviveTime = CreateConVar("sm_revive_time", "10.0", "自动复活时长(5.0-60.0秒)", FCVAR_NOTIFY);
		...
		case MenuAction_Select: 
		{
			switch(position)
			{
				case 0:
				{
					...
					{
						PrintToChat(client, "\x04[提示]\x01对方已经闲置或者退出了游戏！", tpFromName);
					}
				}
				case 1:
				{
...
{
	if (HasSavedLocation(0, 0) && teleEnable)
	{
