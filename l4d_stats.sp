...
public OnPluginStart()
{
  ...
  CreateConVar("l4d_stats_version", PLUGIN_VERSION, "Custom Player Stats Version", FCVAR_NOTIFY|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
  ...
  // Administrative Cvars
  cvar_AdminPlayerCleanLastOnTime = CreateConVar("l4d_stats_adm_cleanoldplayers", "2", "多少个月后玩家信息会被清除 零为关闭", FCVAR_NOTIFY, true, 0.0);
  cvar_AdminPlayerCleanPlatime = CreateConVar("l4d_stats_adm_cleanplaytime", "30", "玩家需要玩多少分钟才会不被清除数据 零为关闭", FCVAR_NOTIFY, true, 0.0);

  // Config/control Cvars
  cvar_EnableRankVote = CreateConVar("l4d_stats_enablerankvote", "1", "允许根据玩家PPM 每分钟点数 投票更换队伍", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_HumansNeeded = CreateConVar("l4d_stats_minhumans", "2", "记录状态所需的最少玩家数", FCVAR_NOTIFY, true, 1.0, true, 4.0);
  cvar_UpdateRate = CreateConVar("l4d_stats_updaterate", "90", "普通感染者数据公告以及更新的时间间隔 秒", FCVAR_NOTIFY, true, 30.0);
  //cvar_AnnounceRankMinChange = CreateConVar("l4d_stats_announcerankminpoint", "500", "Minimum change to points before rank change announcement", FCVAR_NOTIFY, true, 0.0);
  cvar_AnnounceRankChange = CreateConVar("l4d_stats_announcerank", "1", "等级更改时进行公告", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_AnnounceRankChangeIVal = CreateConVar("l4d_stats_announcerankinterval", "60", "等级变更检查间隔", FCVAR_NOTIFY, true, 10.0);
  cvar_AnnouncePlayerJoined = CreateConVar("l4d_stats_announceplayerjoined", "1", "加入的玩家聊天公告", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_AnnounceMotd = CreateConVar("l4d_stats_announcemotd", "1", "聊天通知当天的消息", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_AnnounceMode = CreateConVar("l4d_stats_announcemode", "1", "聊天通知模式 零 关闭 一 仅限玩家 二 仅限玩家与爆头 三 全部公开", FCVAR_NOTIFY, true, 0.0, true, 3.0);
  ...
  //cvar_MaxPoints = CreateConVar("l4d_stats_maxpoints", "500", "Maximum number of points that can be earned in a single map. Normal = x1, Adv = x2, Expert = x3", FCVAR_NOTIFY, true, 500.0);
  cvar_DbPrefix = CreateConVar("l4d_stats_dbprefix", "", "统计表的前缀", FCVAR_NOTIFY);
  //cvar_LeaderboardTime = CreateConVar("l4d_stats_leaderboardtime", "14", "Time in days to show Survival Leaderboard times", FCVAR_NOTIFY, true, 1.0);
  cvar_EnableNegativeScore = CreateConVar("l4d_stats_enablenegativescore", "1", "启用点损失（负分数）", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_FriendlyFireMode = CreateConVar("l4d_stats_ffire_mode", "2", "友伤模式。 0 =正常，1 =冷却，2 =基于损伤", FCVAR_NOTIFY, true, 0.0, true, 2.0);
  cvar_FriendlyFireMultiplier = CreateConVar("l4d_stats_ffire_multiplier", "1.5", "友方火焰伤害倍数（公式：分数=伤害*数）", FCVAR_NOTIFY, true, 0.0);
  cvar_FriendlyFireCooldown = CreateConVar("l4d_stats_ffire_cooldown", "10.0", "友军火焰冷却的时间，以秒为单位", FCVAR_NOTIFY, true, 1.0);
  cvar_FriendlyFireCooldownMode = CreateConVar("l4d_stats_ffire_cooldownmode", "1", "友伤冷却模式。 0 =禁用，1 =玩家特定，2 =常规", FCVAR_NOTIFY, true, 0.0, true, 2.0);

  // Game mode Cvars
  cvar_Enable = CreateConVar("l4d_stats_enable", "1", "启用/禁用所有统计信息跟踪", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_EnableCoop = CreateConVar("l4d_stats_enablecoop", "1", "启用/禁用合作统计信息跟踪", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_EnableSv = CreateConVar("l4d_stats_enablesv", "1", "启用/禁用生还统计信息跟踪", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_EnableVersus = CreateConVar("l4d_stats_enableversus", "1", "启用/禁用对抗统计信息跟踪", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_EnableTeamVersus = CreateConVar("l4d_stats_enableteamversus", "1", "[L4D2] 启用/禁用团队团队对抗统计信息跟踪", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_EnableRealism = CreateConVar("l4d_stats_enablerealism", "1", "[L4D2] 启用/禁用写实统计信息跟踪", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_EnableScavenge = CreateConVar("l4d_stats_enablescavenge", "1", "[L4D2] 启用/禁用清道夫统计信息跟踪", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_EnableTeamScavenge = CreateConVar("l4d_stats_enableteamscavenge", "1", "[L4D2] 启用/禁用团队清道夫统计信息跟踪", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_EnableRealismVersus = CreateConVar("l4d_stats_enablerealismvs", "1", "[L4D2] 启用/禁用写实对抗统计信息跟踪", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_EnableTeamRealismVersus = CreateConVar("l4d_stats_enableteamrealismvs", "1", "[L4D2] 启用/禁用团队写实对抗统计信息跟踪", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_EnableMutations = CreateConVar("l4d_stats_enablemutations", "1", "[L4D2] 启用/禁用突变统计信息跟踪", FCVAR_NOTIFY, true, 0.0, true, 1.0);

  // Game mode depended Cvars
  cvar_RealismMultiplier = CreateConVar("l4d_stats_realismmultiplier", "1.4", "[L4D2] 写实合作得分倍数", FCVAR_NOTIFY, true, 1.0);
  cvar_RealismVersusSurMultiplier = CreateConVar("l4d_stats_realismvsmultiplier_s", "1.4", "[L4D2] 写实对抗幸存者得分倍数", FCVAR_NOTIFY, true, 1.0);
  cvar_RealismVersusInfMultiplier = CreateConVar("l4d_stats_realismvsmultiplier_i", "0.6", "[L4D2] 写实对抗感染者得分倍数", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_EnableSvMedicPoints = CreateConVar("l4d_stats_medicpointssv", "0", "启用生还医疗点数", FCVAR_NOTIFY, true, 0.0, true, 1.0);

  // Infected point Cvars
  cvar_Infected = CreateConVar("l4d_stats_infected", "1", "杀死普通感染者的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Hunter = CreateConVar("l4d_stats_hunter", "2", "杀死Hunter的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Smoker = CreateConVar("l4d_stats_smoker", "3", "杀死Smoker的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Boomer = CreateConVar("l4d_stats_boomer", "5", "杀死Boomer的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Spitter = CreateConVar("l4d_stats_spitter", "5", "[L4D2] 杀死Spitter的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Jockey = CreateConVar("l4d_stats_jockey", "5", "[L4D2] 杀死Jockey的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Charger = CreateConVar("l4d_stats_charger", "5", "[L4D2] 杀死Charger的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_InfectedDamage = CreateConVar("l4d_stats_infected_damage", "2", "获取1点分数所需的给幸存者造成伤害量", FCVAR_NOTIFY, true, 1.0);

  // Misc personal gain Cvars
  cvar_Pills = CreateConVar("l4d_stats_pills", "15", "给予队友药丸的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Adrenaline = CreateConVar("l4d_stats_adrenaline", "15", "[L4D2] 给予队友肾上腺素的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Medkit = CreateConVar("l4d_stats_medkit", "20", "给队友打包的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Defib = CreateConVar("l4d_stats_defib", "20", "[L4D2] 电起一名队友的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_SmokerDrag = CreateConVar("l4d_stats_smokerdrag", "5", "救出被Somker拖拽的队友的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_JockeyRide = CreateConVar("l4d_stats_jockeyride", "10", "[L4D2] 救出被Jockey骑乘的队友的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_ChargerPlummel = CreateConVar("l4d_stats_chargerplummel", "10", "[L4D2] 救出被Charger控制的队友的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_ChargerCarry = CreateConVar("l4d_stats_chargercarry", "15", "[L4D2] 救出被Charger携带的队友的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_ChokePounce = CreateConVar("l4d_stats_chokepounce", "10", "救出被Hunter突袭 / Smoker窒息的队友的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Revive = CreateConVar("l4d_stats_revive", "15", "救起倒地队友的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Rescue = CreateConVar("l4d_stats_rescue", "10", "从复活点救出队友的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Protect = CreateConVar("l4d_stats_protect", "3", "在战斗中保护队友的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_PlayerLedgeSuccess = CreateConVar("l4d_stats_ledgegrap", "15", "导致幸存者挂边的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Matador = CreateConVar("l4d_stats_matador", "30", "[L4D2] 使用近战武器杀死冲锋中的Charger的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_WitchCrowned = CreateConVar("l4d_stats_witchcrowned", "30", "惊扰Witch的基本分数", FCVAR_NOTIFY, true, 1.0);

  // Team gain Cvars
  cvar_Tank = CreateConVar("l4d_stats_tank", "25", "杀死Tank的基本队伍分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Panic = CreateConVar("l4d_stats_panic", "25", "在没有倒地情况下存活过一波尸潮的基本队伍分数", FCVAR_NOTIFY, true, 1.0);
  cvar_BoomerMob = CreateConVar("l4d_stats_boomermob", "10", "在没有倒地情况下存活过一波Boomer尸潮的基本队伍分数", FCVAR_NOTIFY, true, 1.0);
  cvar_SafeHouse = CreateConVar("l4d_stats_safehouse", "10", "到达安全屋的队伍分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Witch = CreateConVar("l4d_stats_witch", "10", "没有惊扰Witch的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_VictorySurvivors = CreateConVar("l4d_stats_campaign", "5", "完成战役的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_VictoryInfected = CreateConVar("l4d_stats_infected_win", "30", "感染者队伍的胜利基本分数", FCVAR_NOTIFY, true, 1.0);

  // Point loss Cvars
  cvar_FFire = CreateConVar("l4d_stats_ffire", "25", "造成队友伤害的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_FIncap = CreateConVar("l4d_stats_fincap", "75", "造成队友伤害的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_FKill = CreateConVar("l4d_stats_fkill", "250", "杀死队友的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_InSafeRoom = CreateConVar("l4d_stats_insaferoom", "5", "让感染者进入安全屋的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_Restart = CreateConVar("l4d_stats_restart", "100", "回合开始的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_MedkitUsedPointPenalty = CreateConVar("l4d_stats_medkitpenalty", "0.1", "每个使用过的Medkit所有幸存者获得积分的分数减少（公式：Score = NormalPoints * (1 - MedkitsUsed * MedkitPenalty))", FCVAR_NOTIFY, true, 0.0, true, 0.5);
  cvar_MedkitUsedPointPenaltyMax = CreateConVar("l4d_stats_medkitpenaltymax", "1.0", "最大分数减少（使用Medkit时，分数减少不会超过此值）", FCVAR_NOTIFY, true, 0.0, true, 1.0);
  cvar_MedkitUsedFree = CreateConVar("l4d_stats_medkitpenaltyfree", "0", "团队幸存者可以免费使用这么多Medkits而不会降低分数", FCVAR_NOTIFY, true, 0.0);
  cvar_MedkitUsedRealismFree = CreateConVar("l4d_stats_medkitpenaltyfree_r", "4", "团队幸存者可以免费使用这么多Medkits而不会在玩现实主义游戏模式时降低分数（-1 =使用l4d_stats_medkitpenaltyfree中的值）", FCVAR_NOTIFY, true, -1.0);
  cvar_MedkitBotMode = CreateConVar("l4d_stats_medkitbotmode", "1", "当bot使用medkit时添加分数减少。 0 =否，1 = Bot将Medkit用于人类玩家，2 = Bot使用Medkit而非自身，3 =是", FCVAR_NOTIFY, true, 0.0, true, 2.0);
  cvar_CarAlarm = CreateConVar("l4d_stats_caralarm", "50", "[L4D2] 触发式汽车警报的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_BotScoreMultiplier = CreateConVar("l4d_stats_botscoremultiplier", "1.0", "在接收机器人相关分数惩罚时使用的乘数。 0 =禁用", FCVAR_NOTIFY, true, 0.0);

  // Survivor point Cvars
  cvar_SurvivorDeath = CreateConVar("l4d_stats_survivor_death", "40", "死幸存者的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_SurvivorIncap = CreateConVar("l4d_stats_survivor_incap", "15", "造成幸存者倒地的基本分数", FCVAR_NOTIFY, true, 1.0);

  // Hunter point Cvars
  cvar_HunterPerfectPounceDamage = CreateConVar("l4d_stats_perfectpouncedamage", "25", "从猛扑中获得的完美突袭（Death From Above）所需造成的伤害", FCVAR_NOTIFY, true, 1.0);
  cvar_HunterPerfectPounceSuccess = CreateConVar("l4d_stats_perfectpouncesuccess", "25", "成功完美突击的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_HunterNicePounceDamage = CreateConVar("l4d_stats_nicepouncedamage", "15", "从突袭中获得的伤害量可以获得Nice Pounce（Pain From Above）所需造成的伤害", FCVAR_NOTIFY, true, 1.0);
  cvar_HunterNicePounceSuccess = CreateConVar("l4d_stats_nicepouncesuccess", "10", "成功的Nice Pounce的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_HunterDamageCap = CreateConVar("l4d_stats_hunterdamagecap", "25", "Hunter储存的伤害上限", FCVAR_NOTIFY, true, 25.0);
  ...
  // Boomer point Cvars
  cvar_BoomerSuccess = CreateConVar("l4d_stats_boomersuccess", "5", "成功呕吐幸存者的基础分数", FCVAR_NOTIFY, true, 1.0);
  cvar_BoomerPerfectHits = CreateConVar("l4d_stats_boomerperfecthits", "4", "获得Boomer完美喷吐和成功点数所需造成的失明的幸存者个数", FCVAR_NOTIFY, true, 4.0);
  cvar_BoomerPerfectSuccess = CreateConVar("l4d_stats_boomerperfectsuccess", "30", "Bommer完美喷涂的成功点数", FCVAR_NOTIFY, true, 1.0);

  // Tank point Cvars
  cvar_TankDamageCap = CreateConVar("l4d_stats_tankdmgcap", "500", "坦克获得伤害分的伤害上线", FCVAR_NOTIFY, true, 150.0);
  cvar_TankDamageTotal = CreateConVar("l4d_stats_bulldozer", "200", "坦克获取推土机奖励成功点数的所需造成的伤害", FCVAR_NOTIFY, true, 200.0);
  cvar_TankDamageTotalSuccess = CreateConVar("l4d_stats_bulldozersuccess", "50", "推土机奖励的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_TankThrowRockSuccess = CreateConVar("l4d_stats_tankthrowrocksuccess", "5", "坦克石头砸中的基本分数", FCVAR_NOTIFY, true, 0.0);

  // Charger point Cvars
  cvar_ChargerRamSuccess = CreateConVar("l4d_stats_chargerramsuccess", "40", "Charger多重冲撞的基本分数", FCVAR_NOTIFY, true, 1.0);
  cvar_ChargerRamHits = CreateConVar("l4d_stats_chargerramhits", "4", "获取Charger多重冲撞所需撞击的幸存者个数", FCVAR_NOTIFY, true, 2.0);

  // Misc L4D2 Cvars
  cvar_AmmoUpgradeAdded = CreateConVar("l4d_stats_deployammoupgrade", "10", "[L4D2] 部署弹药升级的基本分数", FCVAR_NOTIFY, true, 0.0);
  cvar_GascanPoured = CreateConVar("l4d_stats_gascanpoured", "5", "[L4D2] 成功倒入一桶汽油的基本分数", FCVAR_NOTIFY, true, 0.0);

  // Other Cvars
  cvar_Top10PPMMin = CreateConVar("l4d_stats_top10ppmplaytime", "30", "在top10 ppm列表中显示的最短游戏时间（分钟）", FCVAR_NOTIFY, true, 1.0);
  cvar_RankVoteTime = CreateConVar("l4d_stats_rankvotetime", "20", "等待人们投票的时间", FCVAR_NOTIFY, true, 10.0);

  cvar_SoundsEnabled = CreateConVar("l4d_stats_soundsenabled", "1", "在某些事件上播放声音", FCVAR_NOTIFY, true, 0.0, true, 1.0);
