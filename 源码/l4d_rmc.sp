...
public void OnPluginStart()
{
	...
	hKickEnable = CreateConVar("L4D2_Kick_Enable", "1", "[0=关|1=开]是否开启自动踢出多余BOT.1=On,0=Off");
...
public Action DisKickClient(Handle timer)
{
	...
	KickEnable = GetConVarBool(hKickEnable);
	if (KickEnable && playernum + botnum > 4 && botnum > specnum)
	{
		int i = MaxClients;
		while (i > 0 && playernum + botnum > 4 && botnum > specnum)
		{
			if (IsClientInGame(i))
			{
				if (IsFakeClient(i) && GetClientTeam(i) == 2)
				{
					KickClient(i, "由于插件bug，你被当成电脑踢出了QAQ");
					botnum--;
				}
			}
			i--;
...
public int LCreateOneBot(int client)
{
	...
	specnum = StringToInt(aynus, 10);
	botnum = StringToInt(abnus, 10);
	if (botnum < specnum || playernum + botnum < 4)
	{
...
public Action Event_rmcteam(Event event, char[] name, bool dontBroadcast)
{
	if (RAutoBotcheck)
	{
		...
		if (client)
		{
			...
			{
				PrintToChat(client, "\x01您已加入观察者，如果想要重新加入游戏，请输入!jg");
			}
			else if (GetClientTeam(client) == 2)
			{
				PrintToChat(client, "\x01您已成功加入游戏");
			}
		}
	}
}
