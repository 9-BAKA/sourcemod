...
#define WTCH_NONE       0
#define WTCH_HEALTH     1
#define WTCH_GOTSLASH   2
#define WTCH_STARTLED   3
#define WTCH_CROWNER    4
#define WTCH_CROWNSHOT  5
#define WTCH_CROWNTYPE  6
...
enAlarmReasons          g_iLastCarAlarmReason   [MAXPLAYERS + 1];                               // what this survivor did to set the last alarm off
...
public OnPluginStart()
{
    ...
    g_hCvarReport = CreateConVar(           "sm_skill_report_enable" ,      "1", "Whether to report in chat (see sm_skill_report_flags).", 0, true, 0.0, true, 1.0 );
    g_hCvarReportFlags = CreateConVar(      "sm_skill_report_flags" ,       REP_DEFAULT, "What to report skeets in chat (bitflags: 1,2:skeets/hurt; 4,8:level/chip; 16,32:crown/draw; 64,128:cut/selfclear, ... ).", 0, true, 0.0 );
    
    g_hCvarAllowMelee = CreateConVar(       "sm_skill_skeet_allowmelee",    "1", "Whether to count/forward melee skeets.", 0, true, 0.0, true, 1.0 );
    g_hCvarAllowSniper = CreateConVar(      "sm_skill_skeet_allowsniper",   "1", "Whether to count/forward sniper/magnum headshots as skeets.", 0, true, 0.0, true, 1.0 );
    g_hCvarAllowGLSkeet = CreateConVar(     "sm_skill_skeet_allowgl",       "1", "Whether to count/forward direct GL hits as skeets.", 0, true, 0.0, true, 1.0 );
    g_hCvarDrawCrownThresh = CreateConVar(  "sm_skill_drawcrown_damage",  "500", "How much damage a survivor must at least do in the final shot for it to count as a drawcrown.", 0, true, 0.0, false );
    g_hCvarSelfClearThresh = CreateConVar(  "sm_skill_selfclear_damage",  "200", "How much damage a survivor must at least do to a smoker for him to count as self-clearing.", 0, true, 0.0, false );
    g_hCvarHunterDPThresh = CreateConVar(   "sm_skill_hunterdp_height",   "400", "Minimum height of hunter pounce for it to count as a DP.", 0, true, 0.0, false );
    g_hCvarJockeyDPThresh = CreateConVar(   "sm_skill_jockeydp_height",   "300", "How much height distance a jockey must make for his 'DP' to count as a reportable highpounce.", 0, true, 0.0, false );
    g_hCvarHideFakeDamage = CreateConVar(   "sm_skill_hidefakedamage",      "0", "If set, any damage done that exceeds the health of a victim is hidden in reports.", 0, true, 0.0, true, 1.0 );
    g_hCvarDeathChargeHeight = CreateConVar("sm_skill_deathcharge_height","400", "How much height distance a charger must take its victim for a deathcharge to be reported.", 0, true, 0.0, false );
    g_hCvarInstaTime = CreateConVar(        "sm_skill_instaclear_time",     "0.75", "A clear within this time (in seconds) counts as an insta-clear.", 0, true, 0.0, false );
    g_hCvarBHopMinStreak = CreateConVar(    "sm_skill_bhopstreak",          "3", "The lowest bunnyhop streak that will be reported.", 0, true, 0.0, false );
    g_hCvarBHopMinInitSpeed = CreateConVar( "sm_skill_bhopinitspeed",     "150", "The minimal speed of the first jump of a bunnyhopstreak (0 to allow 'hops' from standstill).", 0, true, 0.0, false );
    g_hCvarBHopContSpeed = CreateConVar(    "sm_skill_bhopkeepspeed",     "300", "The minimal speed at which hops are considered succesful even if not speed increase is made.", 0, true, 0.0, false );
    ...
    if ( g_hCvarMaxPounceDistance == INVALID_HANDLE ) { g_hCvarMaxPounceDistance = CreateConVar( "z_pounce_damage_range_max",  "1000.0", "Not available on this server, added by l4d2_skill_detect.", 0, true, 0.0, false ); }
    if ( g_hCvarMinPounceDistance == INVALID_HANDLE ) { g_hCvarMinPounceDistance = CreateConVar( "z_pounce_damage_range_min",  "300.0", "Not available on this server, added by l4d2_skill_detect.", 0, true, 0.0, false ); }
    if ( g_hCvarMaxPounceDamage == INVALID_HANDLE ) { g_hCvarMaxPounceDamage = CreateConVar( "z_hunter_max_pounce_bonus_damage",  "49", "Not available on this server, added by l4d2_skill_detect.", 0, true, 0.0, false ); }
...
stock HandlePop( attacker, victim, shoveCount, Float:timeAlive )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_POP )
    {
        ...
        {
            PrintToChatAll( "\x04%N\x01空爆了\x05%N\x01的bommer.", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x04%N\x01空爆了一只boomer.", attacker );
        }
    }
...
stock HandleLevel( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_LEVEL )
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            PrintToChatAll( "\x04%N\x01用近战击倒了满血\x05%N\x01的冲锋.", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x04%N\x01用近战击倒了满血charger的冲锋.", attacker );
        }
        else {
            PrintToChatAll( "一只charger在冲锋时被杀死了." );
        }
    }
...
stock HandleLevelHurt( attacker, victim, damage )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_HURTLEVEL )
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            PrintToChatAll( "\x04%N\x01用近战击倒了\x05%N\x01的冲锋 (\x03%i\x01点伤害).", attacker, victim, damage );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x04%N\x01用近战击倒了charger的冲锋. (\x03%i\x01点伤害)", attacker, damage );
        }
        else {
            PrintToChatAll( "一只charger在冲锋时被杀死了 (\x03%i\x01点伤害).", damage );
        }
    }
...
stock HandleDeadstop( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_DEADSTOP )
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            PrintToChatAll( "\x04%N\x01推停了\x05%N\x01.", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x04%N\x01推停了一只hunter.", attacker );
        }
    }
...
stock HandleShove( attacker, victim, zombieClass )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_SHOVE )
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            PrintToChatAll( "\x04%N\x01推开了\x05%N\x01.", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x04%N\x01推开了一只特感.", attacker );
        }
    }
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_SKEET )
    {
        if ( attacker == -2 )
        {
            ...
            if ( IS_VALID_INGAME(victim) && !IsFakeClient(victim) ) {
                PrintToChatAll( "\x05%N\x01被队友击杀.", victim );
            } else {
                PrintToChatAll( "\x01一只hunter被队友击杀." );
            }
        }
        else if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            PrintToChatAll( "\x04%N\x01%s击杀\x05%N\x01.",
                    attacker,
        ...
        {
            PrintToChatAll( "\x04%N\x01%s空爆了一只hunter.",
                    attacker,
                    (bMelee) ? "近战-": ((bSniper) ? "爆头-" : ((bGL) ? "榴弹-" : "") )
                );
        }
...
stock HandleNonSkeet( attacker, victim, damage, bool:bOverKill = false, bool:bMelee = false, bool:bSniper = false )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_HURTSKEET )
    {
        ...
        {
            PrintToChatAll( "\x05%N\x01\x04没有\x01被击杀(\x03%i\x01点伤害).%s", victim, damage, (bOverKill) ? "(Would've skeeted if hunter were unchipped!)" : "" );
        }
        else
        {
            PrintToChatAll( "\x01Hunter\x04没有\x01被击杀(\x03%i\x01点伤害).%s", damage, (bOverKill) ? "(Would've skeeted if hunter were unchipped!)" : "" );
        }
    }
...
HandleCrown( attacker, damage )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_CROWN )
    {
        ...
        {
            PrintToChatAll( "\x04%N\x01秒杀了一只witch(\x03%i\x01点伤害).", attacker, damage );
        }
        else {
            PrintToChatAll( "一只妹子被秒杀了." );
        }
    }
...
HandleDrawCrown( attacker, damage, chipdamage )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_DRAWCROWN )
    {
        if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x04%N\x01引秒了一只witch (\x03%i\x01点伤害, \x05%i\x01 chip).", attacker, damage, chipdamage );
        }
        else {
            PrintToChatAll( "一只witch被引秒了(\x03%i\x01点伤害, \x05%i\x01 chip).", damage, chipdamage );
        }
    }
...
HandleTongueCut( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_TONGUECUT )
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            PrintToChatAll( "\x04%N\x01切断了\x05%N\x01的舌头.", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x04%N\x01切断了smoker的舌头.", attacker );
        }
    }
...
HandleSmokerSelfClear( attacker, victim, bool:withShove = false )
{
    ...
    if (    GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_SELFCLEAR &&
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            PrintToChatAll( "\x04%N\x01%s从\x05%N\x01的舌头下自救.", attacker, (withShove) ? "通过推" : "", victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x04%N\x01%s从舌头下自救.", attacker, (withShove) ? "通过推" : "" );
        }
    }
...
HandleRockSkeeted( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_ROCKSKEET )
    {
        ...
        */
        PrintToChatAll( "\x04%N\x01击碎了tank的石头.", attacker );
    }
...
stock HandleHunterDP( attacker, victim, actualDamage, Float:calculatedDamage, Float:height, bool:playerIncapped = false )
{
    ...
    if (    GetConVarBool(g_hCvarReport)
        ...
        {
            PrintToChatAll( "\x04%N\x01高扑了\x05%N\x01 (造成\x03%i\x01点伤害, 高度: \x05%i\x01).", attacker,  victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "一只hunter高扑了\x05%N\x01 (造成\x03%i\x01点伤害, 高度: \x05%i\x01).", victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
    }
...
stock HandleJockeyDP( attacker, victim, Float:height )
{
    ...
    if (    GetConVarBool(g_hCvarReport)
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(attacker) )
        {
            PrintToChatAll( "\x04%N\x01的猴子高扑了\x05%N\x01 (高度: \x05%i\x01).", attacker,  victim, RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "一只猴子高扑了\x05%N\x01 (高度: \x05%i\x01).", victim, RoundFloat(height) );
        }
    }
...
stock HandleDeathCharge( attacker, victim, Float:height, Float:distance, bool:bCarried = true )
{
    ...
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(attacker) )
        {
            PrintToChatAll( "\x04%N\x01用charger%s秒杀了\x05%N\x01 (高度: \x05%i\x01).",
                    attacker,
                    (bCarried) ? "" : "\x03冲锋\x01",
                    victim,
                    RoundFloat(height)
        ...
        {
            PrintToChatAll( "一只charger%s秒杀了\x05%N\x01 (高度: \x05%i\x01).",
                    (bCarried) ? "" : "\x03冲锋\x01",
                    victim,
                    RoundFloat(height) 
                );
    ...
    if ( attacker != pinVictim && GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_INSTACLEAR )
    {
        ...
        if ( fClearTime != -1.0 && fClearTime <= fMinTime )
        {
            if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
            {
                ...
                {
                    PrintToChatAll( "\x04%N\x01将\x05%N\x01从\x04%N\x01(%s)的控制下秒救了下来 (用时%.2f秒).",
                            attacker, pinVictim, victim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
                } else {
                    PrintToChatAll( "\x04%N\x01将一名队友从\x04%N\x01(%s)的控制下秒救了下来 (用时%.2f秒).",
                            attacker, victim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
            ...
            else if ( IS_VALID_INGAME(attacker) )
            {
                if ( IS_VALID_INGAME(pinVictim) )
                {
                    PrintToChatAll( "\x04%N\x01将\x05%N\x01从%s的控制下秒救了下来 (用时%.2f秒).",
                            attacker, pinVictim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
                } else {
                    PrintToChatAll( "\x04%N\x01将一名队友从%s的控制下秒救了下来 (用时%.2f秒).",
                            attacker,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
...
stock HandleBHopStreak( survivor, streak, Float: maxVelocity )
{
    if (    GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_BHOPSTREAK &&
        ...
        PrintToChatAll( "\x04玩家\x03[%N]\x04连跳成功%d次!(最高速度: \x05%.1f\x01).",
                survivor,
                streak,
                maxVelocity
            );
...
// car alarms
stock HandleCarAlarmTriggered(int survivor,int infected,enAlarmReasons reason )
{
    if (    GetConVarBool(g_hCvarReport) && GetConVarInt(g_hCvarReportFlags) & REP_CARALARM &&
            IS_VALID_INGAME(survivor) && !IsFakeClient(survivor)
    ) {
        if ( reason == CALARM_HIT ) {
            PrintToChatAll( "\x05%N\x01触发了一次警报.", survivor );
        }
        else if ( reason == CALARM_TOUCHED )
        {
            ...
            if ( IS_VALID_INFECTED(infected) )
            {
                ...
                {
                    PrintToChatAll( "\x04%N\x01让\x05%N\x01击发触发了一次警报.", infected, survivor );
                }
                else {
                    switch ( GetEntProp(infected, Prop_Send, "m_zombieClass") )
                    {
                        case ZC_SMOKER: { PrintToChatAll( "\x01一只hunter让\x05%N\x01触发了一次警报.", survivor ); }
                        case ZC_JOCKEY: { PrintToChatAll( "\x01一只ockey让\x05%N\x01触发了一次警报.", survivor ); }
                        case ZC_CHARGER: { PrintToChatAll( "\x01一只charger让\x05%N\x01触发了一次警报.", survivor ); }
                        default: { PrintToChatAll( "\x01一只电脑感染者让\x05%N\x01触发了一次警报.", survivor ); }
                    }
                }
            }
            else
            ...
            PrintToChatAll( "\x05%N\x01引起的爆炸引起了警报.", survivor );
        }
        else if ( reason == CALARM_BOOMER )
        {
            if ( IS_VALID_INFECTED(infected) && !IsFakeClient(infected) )
            {
                PrintToChatAll( "\x05%N\x01由于杀死了一只\x04%N\x01的bommer引起了警报.", survivor, infected );
            }
            else
            {
                PrintToChatAll( "\x05%N\x01由于杀死了一只boomer\x04%N\x01引起了警报.", survivor );
            }
        }
        else {
            PrintToChatAll( "\x05%N\x01触发了一次警报.", survivor );
