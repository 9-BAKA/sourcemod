...
int Pressed[MAXPLAYERS+1];
int Entered[MAXPLAYERS+1];
int TotalEntered;
int TotalPressed;
bool TpAllEnable;
new sm_TpAllEnable;
...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_getloc", GetLoc, "获取当前地点");

	hTpallEnable = CreateConVar("sm_tpall_enable", "0", "是否允许tpall指令", FCVAR_NOTIFY);
...
public Action:GetLoc(client, agrs)
{
	new Float:loc[3];
...
public Action:Botton(Handle:event, String:name[], bool:dontBroadcast)
{
	...
	if (TotalPressed == 1)
	{
		PrintToChatAll("\x04%s\x03第一个\x01按下机关,耗时\x03%i分%i秒", clientName, minutes, seconds);
	}
	else
	{
...
public Action:Door(Handle:event, String:name[], bool:dontBroadcast)  // 实体检查
{
	new Client = GetClientOfUserId(GetEventInt(event, "userid", 0));
...
public Action:BottonUsed(Handle:event, String:name[], bool:dontBroadcast)  // Hook检查
{
	...
	int minutes = timeT / 60;
	int seconds = timeT % 60;
	PrintToChatAll("\x04[提示]\x01所有人已经成功通过了检查点,耗时\x03%i分%i秒", minutes, seconds);
	for (int i = 1; i <= MaxClients; i++)
	{
		Entered[i] = 0;
	}
...
public Action:TeleportAll(client, agrs)
{
	...
	if (!Entered[client] && !Pressed[client])
	{
