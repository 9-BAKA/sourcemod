		...
		if (DamageCache[attackerc][i] != 0 && attackerc != i)
		{
			if (IsClientInGame(i) && IsClientConnected(i))
			{
				...
				switch(GetConVarInt(AnnounceType))
				{
					...
					{
						if (IsClientInGame(attackerc) && IsClientConnected(attackerc) && !IsFakeClient(attackerc))
							PrintToChat(attackerc, "\x05[警告]\x04你对\x03%s\x04造成了\x03%d\x04点伤害!",victim,DamageCache[attackerc][i]);
						if (IsClientInGame(i) && IsClientConnected(i) && !IsFakeClient(i))
							PrintToChat(i, "\x05[警告]\x03%s\x04对你造成\x03%d\x04点伤害!",attacker,DamageCache[attackerc][i]);
						if (DamageCache[attackerc][i] >= 10)
							PrintToChatAll("\x05[成吨输出]\x03%s\x04对\x03%s\x04造成了\x03%d\x04点伤害!",attacker,victim,DamageCache[attackerc][i]);
					}
					case 2:
					{
						if (IsClientInGame(attackerc) && IsClientConnected(attackerc) && !IsFakeClient(attackerc))
						...
						if (IsClientInGame(i) && IsClientConnected(i) && !IsFakeClient(i))
							PrintHintText(i, "%s 对你造成 %d 点伤害 ",attacker,DamageCache[attackerc][i]);
					}
					case 3:
					{
						if (IsClientInGame(attackerc) && IsClientConnected(attackerc) && !IsFakeClient(attackerc))
