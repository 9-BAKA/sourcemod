...
ConVar g_hCvarAllow, g_hCvarAlpha, g_hCvarColor, g_hCvarCustom, g_hCvarHalo, g_hCvarLength, g_hCvarMPGameMode, g_hCvarModes, g_hCvarModesOff, g_hCvarModesTog, g_hCvarTransmit, g_hCvarWidth;
int g_iCvarAlpha, g_iCvarColor, g_iCvarCustom, g_iCvarLength, g_iCvarTransmit, g_iCvarWidth;
...
public void OnPluginStart()
{
	...
	g_hCvarAllow =			CreateConVar(	"l4d_glare_allow",		"1",			"0=关闭插件. 1=启用插件.", CVAR_FLAGS );
	g_hCvarAlpha =			CreateConVar(	"l4d_glare_bright",		"155.0",		"手电筒光束的亮度.", CVAR_FLAGS );
	g_hCvarColor =			CreateConVar(	"l4d_glare_color",		"250 250 200",	"手电筒光束的颜色（每个值之间用空格隔开）. RGB (red, green, blue) values (0-255).", CVAR_FLAGS );
	g_hCvarCustom =			CreateConVar(	"l4d_glare_custom",		"2",			"0=使用服务器设置颜色. 1=允许玩家自定义颜色. 2=保存并恢复玩家自定义颜色.", CVAR_FLAGS );
	g_hCvarHalo =			CreateConVar(	"l4d_glare_halo",		"0.4",			"视觉光晕的亮度（默认即可）.", CVAR_FLAGS );
	g_hCvarLength =			CreateConVar(	"l4d_glare_length",		"50",			"手电筒光束的长度.", CVAR_FLAGS );
	g_hCvarModes =			CreateConVar(	"l4d_glare_modes",		"",				"在那些游戏模式中开启此插件（留空等于全部模式，值之间用逗号隔开）. (留空 = 所有).", CVAR_FLAGS );
	g_hCvarModesOff =		CreateConVar(	"l4d_glare_modes_off",	"",				"在那些游戏模式中关闭此插件（留空等于无，值之间用逗号隔开）. (留空 = 无).", CVAR_FLAGS );
	g_hCvarModesTog =		CreateConVar(	"l4d_glare_modes_tog",	"0",			"在那些游戏模式中开启此插件（0=全部 1=合作, 2=生存, 4=对抗, 8=清道夫）. Add numbers together.", CVAR_FLAGS );
	g_hCvarTransmit =		CreateConVar(	"l4d_glare_transmit",	"1",			"0=隐藏自己的光束. 1=显示自己的光束（第一人称时会尝试隐藏）.", CVAR_FLAGS );
	g_hCvarWidth =			CreateConVar(	"l4d_glare_width",		"5",			"手电筒光束的宽度.", CVAR_FLAGS );
	CreateConVar(							"l4d_glare_version",	PLUGIN_VERSION,	"强光手电筒插件版本.", FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	RegConsoleCmd("sm_glare", CmdGlare, "打开光束颜色菜单. 或者:设定颜色. 使用方法:[color name|R G B]. 或者三个值 0-255. 例如: !glare red 或 !glare 255 0 0");

	CreateColors();
	g_hCookie = RegClientCookie("l4d_glare", "光束颜色", CookieAccess_Protected);
